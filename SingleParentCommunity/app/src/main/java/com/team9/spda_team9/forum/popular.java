...
public class popular extends FirestoreRecyclerAdapter<Topic, popular.ViewHolder> {
    private OnListItemClick onListItemClick;

    private popular.OnItemClickListener mListener;

    public interface OnItemClickListener{
        void onItemClick(int position);
    }

    public void setOnItemClickListener(popular.OnItemClickListener listener) {
        mListener = listener;
    }

    public popular(@NonNull FirestoreRecyclerOptions<Topic> options, OnListItemClick onListItemClick) {
        super(options);
    ...
    @NonNull
    @Override
    public popular.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {

        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_topic_single,
                parent,false);
        return new popular.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull popular.ViewHolder holder, int position, @NonNull Topic model) {
